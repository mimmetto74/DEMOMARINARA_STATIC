...
st.sidebar.header("🗺️ Mappa - Info impianto")
map_info_mode = st.sidebar.radio("Visualizzazione info:", ["Popup su mappa","Box sopra la mappa"])
...
with tab4:
    ...
    # Info box above map if chosen
    if map_info_mode == "Box sopra la mappa":
        st.info(f"**Impianto FV**  \nLat: {lat_sidebar:.6f}  |  Lon: {lon_sidebar:.6f}  \nTilt: {st.session_state['tilt']}°  |  Orient: {st.session_state['orient']}°  \nFonte dati meteO: {provider_pref}")
    # Styled popup HTML
    popup_html = f\"\"\"
    <div style="font-size:14px; background: rgba(255,255,255,0.92); padding:8px 10px; border-radius:10px; color:#111;
                box-shadow: 0 2px 8px rgba(0,0,0,0.25);">
      <b>Impianto FV</b><br>
      Lat: {lat_sidebar:.6f} &nbsp; Lon: {lon_sidebar:.6f}<br>
      Tilt: {st.session_state['tilt']}° &nbsp; Orient: {st.session_state['orient']}°
    </div>
    \"\"\"
